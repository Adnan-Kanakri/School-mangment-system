...
exports.updateMyAccount = async (req, res, next) => {
  ...
  const findAdmin = await teacherService.findById(user.teacherId);
    ...
    let user2 = await teacherService.updatePhone(findAdmin, phone);
...
//       });
//     }
//     let activeUser = await teacher.findById(accountId);
...
exports.deleteAccount = async (req, res, next) => {
  ...
  try {
    ...
    if (adminUser.permission != "9") {
      const error = new Error(
        "you dont have the permission to delete this account!"
      );
      error.statusCode = 401;
      return res.status(401).send({
        message: error.message,
    ...
    let activeUser = await teacher.findByIdAndDelete(accountId);
    res.status(201).json({
...
exports.getTeacherInfo = async (req, res, next) => {
  ...
  try {
    ...
    let user = await teacher.findById(teacherId);
    if (!user) {
